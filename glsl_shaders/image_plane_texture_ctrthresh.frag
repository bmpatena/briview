...
//        sc_temp= (im_color-sc_lut.z)/(sc_lut.w-sc_lut.z);
  //      index=2;
        ...
        color = vec4( r_lut[index] + sc_temp * (r_lut[index+1]-r_lut[index]) , g_lut[index]+ sc_temp * (g_lut[index+1]-g_lut[index]), b_lut[index] + sc_temp * (b_lut[index+1]-b_lut[index]),a_lut[index] + sc_temp * (a_lut[index+1]-a_lut[index]));
        else if (index == 3)
        {
        color = vec4( r_lut[index] + sc_temp * (r_lut_last.x-r_lut[index]) , g_lut[index]+ sc_temp * (g_lut_last.x-g_lut[index]), b_lut[index] + sc_temp * (b_lut_last.x-b_lut[index]),a_lut[index] + sc_temp * (a_lut_last.x-a_lut[index]));
        }else if (index==4)
        {
        color = vec4( r_lut_last.x + sc_temp * (r_lut_last.y-r_lut_last.x) , g_lut_last.x + sc_temp * (g_lut_last.y-g_lut_last.x), b_lut_last.x + sc_temp * (b_lut_last.y-b_lut_last.x),a_lut_last.x + sc_temp * (a_lut_last.y-a_lut_last.x));
        }
